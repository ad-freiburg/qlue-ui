...
export async function executeQuery(editorApp: EditorApp, languageClient: MonacoLanguageClient) {
  ...
  const acquireShareLink = async () => {
    ...
    if (response.ok) {
      ...
      if (!window.location.search.includes(result.queryString)) {
        const path = NO_SLUG_MODE ? '' : window.location.pathname.split('/').slice(0, 2).join('/');
  ...
  Promise.all([
    processQuery(
      query,
      operationType,
      parseInt(document.getElementById('maxSendOnFirstRequest')!.textContent!),
      executeButton
    ).finally(() => {
